(async function () {
    ...
    const routeActivateHandlers = new Map();
    function onRouteActivate(route, handler) {
        if (!route) return;
        if (!routeActivateHandlers.has(route)) {
            routeActivateHandlers.set(route, new Set());
        }
        if (typeof handler === 'function') {
            routeActivateHandlers.get(route).add(handler);
        }
    }
    function notifyRouteActivated(route) {
        if (!routeActivateHandlers.has(route)) return;
        routeActivateHandlers.get(route).forEach((handler) => {
            try {
                handler(route);
            } catch (err) {
                console.error(err);
    ...
    const defaultRoute = initialActive ? initialActive[0] : (navItems[0]?.dataset.route || 'daily-summary');
    ...
    const controllers = {};
    const reportRoutes = ['daily-summary', 'detailed-worklogs', 'due-issues', 'quarter-report'];
    reportRoutes.forEach((route) => {
        controllers[route] = initReports;
    });
    ...
    async function initReports() {
        if (document.body.dataset.reportsReady === 'pending' || document.body.dataset.reportsReady === 'true') {
            return;
        }

        document.body.dataset.reportsReady = 'pending';
        ...
        try {
            ...
            const summaryTable = $('#dailySummaryView #results');
            const summaryBody = summaryTable?.querySelector('tbody');
            const detailedBody = $('#detailedWorklogsView #detailedWorklogsTable tbody');
            const dueTableBody = $('#dueIssuesView #dueThisMonthTable tbody');
            const quarterTableBody = $('#quarterReportView #quarterReportTable tbody');
            const saveBtn = $('#save');

            if (!baseUrl || !baseUrlWrap || !usernameSelect || !jYear || !jMonth || !timeOffHours || !footerTotals || !summaryTable || !summaryBody || !detailedBody || !dueTableBody || !quarterTableBody || !saveBtn) {
                throw new Error('Report views missing required elements.');
            }
            ...
            jMonth.value = String(settings.defaultJMonth || 1);
            timeOffHours.value = settings.defaultTimeOffHours != null ? String(settings.defaultTimeOffHours) : '0';
            ...
            const summaryPlaceholder = '<div class="footer-grid"><span class="pill">Totals here…</span></div>';
            footerTotals.innerHTML = summaryPlaceholder;
            let lastResult = null;
            let fetchToken = 0;
      ...
      `;
        ...
        `;
            ...
            function renderDueIssues(res) {
                ...
                if (!issues.length) {
                    dueTableBody.innerHTML = '<tr><td colspan="8">—</td></tr>';
                    return;
                }
        ...
        `;
            ...
            async function triggerFetch({ reason = 'route' } = {}) {
                const requestId = ++fetchToken;
                try {
                    const selection = await pushSelection();
                    const res = await window.appApi.scanNow(selection);
                    if (requestId !== fetchToken) return;
                    render(res);
                } catch (err) {
                    console.error('Failed to fetch reports', err, reason);
                    if (requestId !== fetchToken) return;
                    render({ ok: false });
                }
            }

            jYear.addEventListener('input', () => {
            ...
            jYear.addEventListener('change', () => {
                triggerFetch({ reason: 'year-change' });
            });
            jMonth.addEventListener('change', () => {
                triggerFetch({ reason: 'month-change' });
            });
            usernameSelect.addEventListener('change', () => {
                triggerFetch({ reason: 'user-change' });
            });
            ...
            saveBtn.addEventListener('click', async () => {
                ...
                await enforceUserVisibility();
                await triggerFetch({ reason: 'save' });
            });

            window.appApi.onScanResult((res) => {
                if (!res?.ok) return;
                const cur = getSelection();
                if (res.jYear === cur.jYear && res.jMonth === cur.jMonth && (!res.username || res.username === cur.username)) {
                    render(res);
                }
            });

            reportRoutes.forEach((route) => {
                onRouteActivate(route, () => {
                    triggerFetch({ reason: `route:${route}` });
                });
            });

            await triggerFetch({ reason: 'init' });
            document.body.dataset.reportsReady = 'true';
        } catch (err) {
            document.body.dataset.reportsReady = '';
            console.error('Failed to initialise reports', err);
        }
    }
})();
