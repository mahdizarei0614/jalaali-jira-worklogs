(async () => {
    ...
    async function fetchAssignedIssues({ baseUrl, headers, username }) {
        ...
        const fieldKeys = {
            sprint: findFieldKeyByName(names, 'Sprint') || findFieldKeyByName(names, 'Sprints'),
            board: findFieldKeyByName(names, 'Board') || findFieldKeyByName(names, 'Boards'),
        ...
        for (const issue of issues) {
            ...
            const sprintValues = (() => {
                if (fieldKeys.sprint && fields[fieldKeys.sprint] != null) {
                    const values = normaliseFieldValues(fields[fieldKeys.sprint]);
                    if (values.length) return values;
                }
                const extracted = extractSprintNames(issue);
